 ...
 * @dev A secure, decentralized freelance marketplace with escrow protection and dispute resolution
 * @notice This contract handles job postings, freelancer assignments, automated payments, and dispute resolution
 */
contract FreelanceMarketplace {
    ...
    /// @notice Emitted when a client registers
    ...
    function registerAsClient() external {
        require(!registeredClients[msg.sender], "Already registered as client");
     ...
     * - Deadline must be in the future (at least 1 hour)
    ...
    function completeJobAndPay(uint256 _jobId) external onlyClient(_jobId) jobExists(_jobId) {
        ...
        // Then perform external call (safe from reentrancy)
    ...
    function raiseDispute(uint256 _jobId) external jobExists(_jobId) {
        ...
        require(
            job.client == msg.sender || job.freelancer == msg.sender,
            "Only client or freelancer can raise dispute"
        );
        require(job.status == JobStatus.InProgress, "Job must be in progress to dispute");
        require(!job.isPaid, "Cannot dispute paid job");
    ...
    /**
     * @notice Cancel a job (only if not started or in dispute)
     * @param _jobId Job ID to cancel
     * 
     * Requirements:
     * - Only client can cancel
     * - Job must be open or disputed
     * - Job must not have been paid
     */
